  ...
  <div id="newsTrends">
    <template v-for="item in newsList">
      <el-row class="newsList">
          <router-link :to="{path: '/newsDetails', query: {id: item.id, type: 1}}">
          ...
          <el-col :xs="24" :sm="24" :md="12" :lg="15" class="newsContent">
            <router-link :to="{path: '/newsDetails', query: {id: item.id, type: 1}}"><h3>{{ item.title }}</h3></router-link>
            <h4>{{ item.add_time }}</h4>
            <p>{{ item.content }}</p>
            <div class="newsParticulars">
              <router-link :to="{path: '/newsDetails', query: {id: item.id, type: 1}}">了解更多</router-link>
            </div>
          </el-col>
      </el-row>
    </template>
...
<script>
  import { listCompanyDynamic } from '../axios/api.js'
  import '../assets/less/newsTrends.less'
  export default{
    name: 'newsTrends',
    data () {
      return {
    ...
    methods: {
      ...
      getNewsList (currentPage) {
        const _this = this
        this.$axios.post(listCompanyDynamic, {
